...
from src.schemas.request.work_order import DispatchRequest, StatusUpdateRequest
...
@router.get("")
async def all_orders(status: str | None = None, area: str | None = None, page: int = Query(1, ge=1), page_size: int = Query(20, ge=1, le=100), _: User = Depends(require_roles("admin")), session: AsyncSession = Depends(db_session)):
    items, total = await WorkOrderDAO(session).list(status=status, area=area, page=page, page_size=page_size)
