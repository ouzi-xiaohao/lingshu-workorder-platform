...
class WorkOrderDAO:
    ...
    async def list_pending_human_review(self, page: int = 1, page_size: int = 20) -> tuple[list[WorkOrder], int]:
        pending = exists().where(
            WorkOrderEvent.work_order_id == WorkOrder.id,
            WorkOrderEvent.action == "needs_human_review",
        )
        resolved = exists().where(
            WorkOrderEvent.work_order_id == WorkOrder.id,
            WorkOrderEvent.action == "human_review_resolved",
        )
        filters = [WorkOrder.is_deleted.is_(False), WorkOrder.status == "待人审", pending, ~resolved]
        total = await self.session.scalar(select(func.count(WorkOrder.id)).where(*filters)) or 0
        rows = await self.session.scalars(
            select(WorkOrder)
            .where(*filters)
    ...
    async def requires_human_review(self, work_order_id: int) -> bool:
        order = await self.session.scalar(
            select(WorkOrder).where(WorkOrder.id == work_order_id, WorkOrder.is_deleted.is_(False))
        )
        return order is not None and order.status == "待人审"
